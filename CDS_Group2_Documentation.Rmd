...
ov_data_total <- ov_data_total %>% select(
  ...
  -time_to_recurrence, -debulking, -BRCA_mutation) # remove non-important variables
# as determined by EDA

# For loading completed dataframe instead of running
# spatial statistic calculations
# ov_data_total <- read.csv("data/Ov_data_final.csv", stringsAsFactors=T)
```

Add second-order spatial statistic columns to data
...
# add each possible cell type combination at different radii to data
for (str in strings) {
  y <- paste0(str, "_K20")
  a <- paste0(str, "_K30")
  b <- paste0(str, "_K40")
...
# add each cell type at different radii to data
for (str in phenos) {
  y <- paste0(str, "_K20")
  a <- paste0(str, "_K30")
  b <- paste0(str, "_K40")
...
for (id in 1:nrow(ov_unique)) {
  ...
  # first for loop calculates the K-cross functions for each combination
  for (i in 1:10) {
    # check for counts
  ...
  # second for loop calculates K-estimates values for each cell type
  for (i in 1:5) {
    # check for counts
    cell_type <- phenos[i]
